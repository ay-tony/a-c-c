...
inline constexpr const std::string_view raw_type_name =
    get_template_function_name_use_decay_type<unique_name_for_auto_detect_prefix_and_suffix_length_0123456789_struct_>();
...
template <class... Ts, class TExpr> constexpr auto is_valid(TExpr expr) -> decltype(expr(declval<Ts...>()), bool()) { return true; }
template <class...> constexpr auto is_valid(...) -> bool { return false; }

template <class T> inline constexpr auto is_range_v = is_valid<T>([](auto t) -> decltype(t.begin(), t.end(), void()) {});
...
#else
template <class From, class To> constexpr auto is_convertible(int) -> decltype(bool(To(declval<From>()))) { return true; }
...
struct fatal {
  template <class T> [[nodiscard]] inline auto operator()(const T &t) const { return detail::fatal_{t}; }
};
struct cfg {
...
template <class T> [[nodiscard]] constexpr auto get_impl(const T &t, int) -> decltype(t.get()) { return t.get(); }
template <class T> [[nodiscard]] constexpr auto get_impl(const T &t, ...) -> decltype(auto) { return t; }
...
struct is_stream_insertable<T, std::void_t<decltype(std::declval<std::ostream &>() << detail::get(std::declval<T>()))>>
    : std::true_type {};
template <typename T> inline constexpr bool is_stream_insertable_v = is_stream_insertable<T>::value;
} // namespace type_traits
    ...
    for (const auto &[suite_name, suite_result] : results_) {
      if (suite_result.fails) {
        err_stream << "\n========================================================"
                   ...
                   << color_.none << '\n'
                   << "asserts: " << (suite_result.assertions) << " | " << suite_result.passed << " passed" << " | " << color_.fail
...
struct test {
  ...
  template <class... Ts> constexpr auto operator=(test_location<void (*)()> _test) {
    on<Ts...>(events::test<void (*)()>{
        .type = type, .name = std::string{name}, .tag = tag, .location = _test.location, .arg = none{}, .run = _test.test});
    return _test.test;
  }
  ...
  constexpr auto operator=(Test _test) -> typename type_traits::identity<Test, decltype(_test())>::type {
    on<Test>(events::test<Test>{
        .type = type, .name = std::string{name}, .tag = tag, .location = {}, .arg = none{}, .run = static_cast<Test &&>(_test)});
    return _test;
  }
...
template <char... Cs> [[nodiscard]] constexpr auto operator""_i() { return detail::integral_constant<math::num<int, Cs...>()>{}; }

template <char... Cs> [[nodiscard]] constexpr auto operator""_s() { return detail::integral_constant<math::num<short, Cs...>()>{}; }

template <char... Cs> [[nodiscard]] constexpr auto operator""_c() { return detail::integral_constant<math::num<char, Cs...>()>{}; }

template <char... Cs> [[nodiscard]] constexpr auto operator""_sc() {
  return detail::integral_constant<math::num<signed char, Cs...>()>{};
}

template <char... Cs> [[nodiscard]] constexpr auto operator""_l() { return detail::integral_constant<math::num<long, Cs...>()>{}; }
  ...
  return [f, t](const auto name) {
    apply(
        [f, name](const auto &...args) {
          (detail::on<F>(
               events::test<F, Ts>{.type = "test", .name = std::string{name}, .tag = {}, .location = {}, .arg = args, .run = f}),
           ...);
        },
        t);
  };
...
template <class T> constexpr auto operator%(const T &t, const decltype(_t) &) { return detail::value<T>{t}; }
...
template <class TExpr> [[nodiscard]] constexpr auto throws(const TExpr &expr) { return detail::throws_<TExpr>{expr}; }

template <class TExpr> [[nodiscard]] constexpr auto nothrow(const TExpr &expr) { return detail::nothrow_{expr}; }
#endif

#if __has_include(<unistd.h>) and __has_include(<sys/wait.h>)
template <class TExpr> [[nodiscard]] constexpr auto aborts(const TExpr &expr) { return detail::aborts_{expr}; }
#endif
...
template <
    ...
    typename = type_traits::requires_t<type_traits::is_stream_insertable_v<TLhs> and type_traits::is_stream_insertable_v<TRhs>>>
[[nodiscard]] constexpr auto eq(const TLhs &lhs, const TRhs &rhs) {
  return detail::eq_{lhs, rhs};
}
template <
    class TLhs, class TRhs, class TEpsilon,
    typename = type_traits::requires_t<type_traits::is_stream_insertable_v<TLhs> and type_traits::is_stream_insertable_v<TRhs>>>
...
template <class T> [[nodiscard]] constexpr auto mut(const T &t) noexcept -> T & { return const_cast<T &>(t); }
